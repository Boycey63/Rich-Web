...
var splitList = document.getElementById("splits-list");

var counter = { state: 'INACTIVE', m: 0, s: 0, ms: 0 };

function counterStr() {
	var min = counter.m.toString();
	var sec = counter.s.toString();
	var mils = counter.ms.toString();
...
var startBtn$ = _Rx.Observable.fromEvent(startBtn, 'click');
startBtn$.map(start).subscribe();
var stopBtn$ = _Rx.Observable.fromEvent(stopBtn, 'click');
stopBtn$.map(stop).subscribe();
var splitBtn$ = _Rx.Observable.fromEvent(splitBtn, 'click');
splitBtn$.map(split).subscribe();
var resetBtn$ = _Rx.Observable.fromEvent(resetBtn, 'click');
...
function split() {
	...
	var pos = newRepoItem.parentNode.childElementCount;
...
var timer$ = _Rx.Observable.interval(10).map(function incr() {
...
function showClock() {
	...
	function drawSeconds() {
		...
		ctx.lineTo(centerPointX + Math.cos(angle) * handLength / 1.2, centerPointY + Math.sin(angle) * handLength / 1.2);
	...
	function drawMinutes() {
		...
		ctx.beginPath();
		ctx.moveTo(centerPointX, centerPointY);
		ctx.lineTo(centerPointX + Math.cos(angle) * handLength / 1.5, centerPointY + Math.sin(angle) * handLength / 1.5);
